...
class MyTopo(Topo):
    def __init__(self, *args, **kwargs):
        ...
        self.max_bg_clients_per_edge = kwargs.get('max_bg_clients_per_edge', 1)
        ...
        self.no_core_switches = kwargs.get('no_core_switches', 2)
        ...
        self.bg_host_limits = dict(cpu_period=100000, cpu_quota=10000, mem_limit="128m", memswap_limit="256m")
    ...
    def build(self, *args, **params):
        ...
        # fl_server
        fl_server = self.addHost('flserver', ip=f"10.0.0.100", mac="00:00:00:00:00:AA",
                                 **self.containernet_kwargs)
        self.addLink(fl_server, core_switches[0], bw=1000)

        # Intermediate levels
        intermediate_levels = self.create_middle_switches(core_switches)
        ...
        bg_edge_switches = self.create_edge_switches(core_switches + random.choices(agg_switches, k=3),
                                                     num_bg_edge_switches, "bg")
    ...
    def create_clients(self, edge_switches, client_type):
        ...
        link_config = {"bw": 100}  # self.link_configs[self.links_by_level[-1]]
        for i, client in enumerate(clients):
            self.addLink(client, edge_switches[i % len_edge], **link_config)

        return clients

    def create_edge_switches(self, agg_switches, number_edge_switches, switch_type):
        ...
        link_config = {"bw": 100}  # self.link_configs[self.links_by_level[-2]]
    ...
    def create_agg_switches(self, valid_intermediate_levels, num_agg):
        ...
        link_config = {"bw": 100}  # self.link_configs[self.links_by_level[-3]]
    ...
    def create_middle_switches(self, core_switches):
        ...
        for level in range(self.number_of_levels):
            ...
            level_switches = [self.addSwitch(f'is{level + 1}_{i + 1}', dpid=f"2{level}{i}", **self.switch_config)
            ...
            for prev_switch in previous_level_switches:
                current_switches = random.sample(level_switches, k=random.randint(2, len(level_switches)))
    ...
    def create_core_switches(self):
        core_switches = [self.addSwitch(f'cs{i + 1}', dpid=f"10{i}", **self.switch_config) for i in
                         range(self.no_core_switches)]  # Example with 2 core switches
        # Create all-to-all connections between core switches
        link_config = {
            "bw": 100}  # self.link_configs[self.links_by_level[0]]  # starting from one to skip core connection
        for i in range(len(core_switches) - 1):
            self.addLink(core_switches[i], core_switches[i + 1], **link_config)
        self.addLink(core_switches[-1], core_switches[0], **link_config)
